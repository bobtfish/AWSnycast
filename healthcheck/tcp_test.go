...
func TestHealthcheckTcpTLSSkipVerify(t *testing.T) {
	...
	server_key := filepath.Join(dir, "server_test.key")
	server_pem := filepath.Join(dir, "server_test.pem")

	cer, _ := tls.LoadX509KeyPair(server_pem, server_key)
...
func TestHealthcheckTcpTLSX509(t *testing.T) {
	dir, _ := filepath.Abs(filepath.Dir(os.Args[0]))
	cacert := filepath.Join(dir, "ca_test.pem")
	server_key := filepath.Join(dir, "server_test.key")
	server_pem := filepath.Join(dir, "server_test.pem")
